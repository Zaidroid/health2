      ...
      >
        ...
        <div className="p-6">
            ...
            >
              ...
              <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-lg overflow-hidden">
                {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                  <div
                    key={day}
                    className="bg-indigo-50 py-2 text-center text-sm font-semibold text-indigo-900"
                  >
                    {day}
                  </div>
                ))}
                {days.map((day, dayIdx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`
                        relative bg-white py-6 px-3 cursor-pointer transition-all duration-200
                        ${isToday(day) ? 'bg-indigo-50' : ''}
                        ${!isSameMonth(day, currentDate) ? 'text-gray-400' : ''}
                        ${isSelected ? 'ring-2 ring-indigo-600 z-10' : ''}
                        ${workout ? 'hover:bg-indigo-50' : 'hover:bg-gray-50'}
                      `}
                    ...
                    >
                      <time
                        ...
                        className={`
                          block text-sm mb-1
                          ${isToday(day) ? 'text-white bg-indigo-600 w-6 h-6 rounded-full flex items-center justify-center mx-auto' : ''}
                        `}
                      >
                        {format(day, 'd')}
                      </time>
          ...
          <AnimatePresence>
            {selectedDate && (
              <motion.div 
                className="mt-8 bg-white p-6 rounded-xl border border-indigo-100 shadow-sm"
              />
            )}
          </AnimatePresence>
        </div>
